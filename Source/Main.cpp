...
int main(int argv, char** argc)
{
   ...
   g_Engine->Init("engine.cfg");
	...
	// Civ1 mouse pointer: torch sprite from SP257 at (113, 33), size 11×13
	// (Images/pointer.png; regenerated by tools/slice_civ_terrain.py).
	HideCursor();
   ...
   g_StateMachine->RegisterState(STATE_TITLESTATE, titleState, "TitleState");
	...
	MapViewerState* mapViewerState = new MapViewerState();
	mapViewerState->Init("");
	g_StateMachine->RegisterState(STATE_MAPVIEWERSTATE, mapViewerState, "MapViewerState");

	CustomizeWorldState* customizeWorldState = new CustomizeWorldState();
	customizeWorldState->Init("");
	g_StateMachine->RegisterState(STATE_CUSTOMIZEWORLDSTATE, customizeWorldState, "CustomizeWorldState");

	DifficultyState* difficultyState = new DifficultyState();
	difficultyState->Init("");
	g_StateMachine->RegisterState(STATE_DIFFICULTYSTATE, difficultyState, "DifficultyState");

	FactionSelectState* factionSelectState = new FactionSelectState();
	factionSelectState->Init("");
	g_StateMachine->RegisterState(STATE_FACTIONSELECTSTATE, factionSelectState, "FactionSelectState");
